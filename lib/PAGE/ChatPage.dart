...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _sendMessage(String message) async {
    ...
    _scrollToBottom(); // NEW: Scroll after adding user message
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _scrollToBottom(); // NEW: Scroll after adding "typing..." indicator
        ...
        response.stream.transform(utf8.decoder).listen((chunk) {
          ...
          _scrollToBottom(); // NEW: Scroll after updating bot's message
        }, onDone: () {
          print("Stream finished");
          widget.onMessagesUpdated(widget.messages);
        }, onError: (error) {
          ...
          _scrollToBottom(); // NEW: Scroll after error message
        });
      } else {
        setState(() {
          widget.messages.add("Error: Unable to get response from API.");
        });
        _scrollToBottom(); // NEW: Scroll after API error
      }
    } catch (e) {
      setState(() {
        widget.messages.add("Error: Unable to connect to the server.");
      });
      _scrollToBottom(); // NEW: Scroll after connection error
    }
  }
